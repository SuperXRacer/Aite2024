...
const model = genAI.getGenerativeModel({
  ...
  async function run() {
    ...
    const chatSession = model.startChat({ generationConfig }); // Start outside the loop

    rl.prompt(); // Initial prompt for the user
    ...
    rl.on('line', async (userInput) => {
        ...
        const message = await chatSession.sendMessage(userInput);
        console.log('AiteAI: ',message.response.text());
        rl.prompt(); // Prompt for the next user input
    });
